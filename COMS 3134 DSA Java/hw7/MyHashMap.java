...
public class MyHashMap<K extends Comparable<K>, V> implements MyMap<K, V> {
    ...
    public V get(K key) {
        int index = hash(key);
        Entry<K,V> n = table[index % table.length];
    ...
    public V put(K key, V value) {
        if(numEntries+1 > MAX_LOAD_FACTOR*table.length){
            if(table.length < 222461) rehash();
        }
        int index = hash(key);
        Entry<K,V> n = table[index % table.length];
        while(n != null){
        ...
        Entry<K,V> oldHead = table[index % table.length];
        Entry<K,V> insert = new Entry<K,V>(key, value);
        insert.next = oldHead;
        table[index % table.length] = insert;
        numEntries++;
        return null;
    }
    ...
    public V remove(K key) {
        ...
        if(prev==null){
            V out = n.value;
            if(n.next==null) table[index % table.length] = null;
            else table[index % table.length] = n.next;
            numEntries--;
            return out;
        }else{
            V out = n.value;
